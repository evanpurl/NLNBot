...
from util.databasefunctions import get, insert
...
class SEcommands(commands.Cog):
    ...
    async def factionlead(self, interaction: discord.Interaction, role: discord.Role, user: discord.User):
        try:
            ...
            if leadrole:
                ...
                await insert(self.bot.database, f"""REPLACE INTO SE_955962668756385792(userid, roleid) VALUES( {user.id}, {role.id})""")
    ...
    async def factionadd(self, interaction: discord.Interaction, user: discord.User):
        try:
            leader = await get(self.bot.database, f""" SELECT roleid FROM SE_955962668756385792 WHERE userid={interaction.user.id} """)
            if leader:
